...
def cargar_modelo_y_métricas(nombre_modelo):
    if nombre_modelo == "rfm_xgboost":
        ...
        with open("dashboardData/LSTM/metricsLSTM.json", "r") as f:
            metrics = json.load(f)
        ...
        # Código para otros modelos adicionales si existen
        return None, None
...
with col3:
    ...
    if nombre_modelo == "rfm_xgboost":
        ...
        fig_loss.update_layout(title="Gráfica de Pérdida durante el Entrenamiento", xaxis_title="Épocas", yaxis_title="Loss", plot_bgcolor=colores['fondo'])
        st.plotly_chart(fig_loss)
...
with col4:
    ...
    if nombre_modelo == "rfm_xgboost":
        ...
        # Agrega lógica específica para ROC de LSTM si está disponible
        pass
...
with col6:
    ...
    if nombre_modelo == "rfm_xgboost":
        ...
        # Adaptar para el modelo LSTM
        X_valid = pd.DataFrame(model_records['X_valid'])  # Asegúrate de cargar X_valid correctamente
        y_valid = model_records['y_valid']
        y_proba = modelo.predict(X_valid).flatten()
        y_pred_adjusted = (y_proba >= threshold).astype(int)
